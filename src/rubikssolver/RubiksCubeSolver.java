...
public class RubiksCubeSolver {
	...
	public static void main(String[] args) throws InterruptedException {
		if (args.length == 1) {
			try {
				if (checkFile(args[0]))
					cube = new RubiksCube(readFile(args[0]));
		...
		try {
			do {
				...
				case localSerachLetters:
					...
					if (s != null) {
						LocalSearchAlgorithm localSearch = new LocalSearchAlgorithm(cube, s,
								(int) (4 * Math.pow(s.size(), 4)));
				...
				case geneticAlgorithmLetters:
					s = getRandomSolution();
					if (s != null) {
						 GeneticAlgorithm geneticAlgorithm = new  GeneticAlgorithm(cube, s,  s.size()*5, s.size()*s.size()*4, 0.7f, 0.2f);
	...
	private static String displayMenu() throws IOException {
		...
		do {
			...
			if (cube != null) {
				...
				System.out.printf(" (%s) Résoudre le cube par l'algorithme profondeur d'abord%n",
						solveByDepthFirstLetter);
			...
			if (!acceptableChoices.contains(choice)) {
				...
				&& !choice.equals(solvedCubeLetter))));
		return choice;
	}
	...
	private static void startAndWaitThenShowExecutionTime(SolvingAlgorithm algorithm) throws IOException {
		try {
			...
			System.out.printf(
					"Temps d'exécution : %.3f s | Nombre d'états visités : %d | Nombre d'états abandonés : %d%n",
					algorithm.getExecutionTime(), algorithm.getTotalStatesCount(), algorithm.getAbandonedStatesCount());
	...
	private static void startMetaHeuristicAndWait(MetaHeuristicAlgorithm algorithm) {
		try {
			algorithm.start();
			algorithm.join();
			System.out.printf("%s :%n%s, Qualité : %.2f%%%nTemps d'exécution : %.3f s | Nombre d'itérations : %d%n",
					(algorithm.solutionFound() ? "Solution trouvée" : "Aucune solution touvée, la meilleure"),
					algorithm.getSolution(), algorithm.getQuality() * 100, algorithm.getExecutionTime(),
